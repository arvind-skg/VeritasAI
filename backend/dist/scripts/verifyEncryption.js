...
import { encryptData, decryptData, isEncrypted, } from "../services/security.js";
import { ensureDefaultOrganization, createAgent, saveDecision, getEvent, listAuditLogs, findActiveCredentialByKey, } from "../services/db.js";
...
async function runVerification() {
    ...
    if (!isEncrypted(encrypted))
        throw new Error("Ciphertext missing enc:v1 prefix");
    ...
    for (const a of rawAgents) {
        if (!isEncrypted(a.apiKey))
            throw new Error(`Agent ${a.id} apiKey is not encrypted or null!`);
    }
    ...
    for (const log of rawAudit) {
        if (!isEncrypted(log.detailsJson))
            throw new Error(`AuditLog ${log.id} detailsJson is not encrypted!`);
    }
    ...
    const inputData = fetchedEvent?.inputJson;
