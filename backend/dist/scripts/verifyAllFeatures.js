 ...
 */
import { verifyActionChain, synthesizeDefaultActionChain, generateSelectiveCommitments, createSelectiveDisclosureProof, verifySelectiveDisclosureProof, generateSignedTreeHead, verifyConsistency, generateHybridSignature, verifyHybridSignature, anchorToExternalAuthority, verifyExternalTimestamp, } from "../services/cryptography/index.js";
import { ensureDefaultOrganization, createAgent, saveDecision, getEvent, getEventCausalChain, } from "../services/db.js";
async function runAllTests() {
    ...
    if (actionChain.length !== 9)
        throw new Error(`Expected 9 stages, got ${actionChain.length}`);
    const chainCheck = verifyActionChain(actionChain);
    if (!chainCheck.valid)
        throw new Error(`Action chain verification failed: ${chainCheck.error}`);
    ...
    if (tamperedCheck.valid)
        throw new Error("Tampered action chain was erroneously accepted!");
    ...
    if (!proofVerification.valid)
        throw new Error(`Selective disclosure verification failed: ${proofVerification.error}`);
    if (proofVerification.verifiedFields.length !== 2)
        throw new Error("Verified field count mismatch");
    ...
    if (forgedCheck.valid)
        throw new Error("Forged selective field was erroneously accepted!");
    ...
    if (sth.treeSize !== 4 || !sth.rootHash)
        throw new Error("Invalid STH generated");
    ...
    if (!consistencyOk.isConsistent)
        throw new Error("Valid append rejected by consistency proof");
    ...
    if (deletionAttack.isConsistent)
        throw new Error("Adversarial deletion attack was not detected!");
    ...
    if (tamperedSigCheck.valid)
        throw new Error("Tampered decision was accepted by hybrid signer!");
    ...
    if (!tsaCheck.valid)
        throw new Error(`TSA verification failed: ${tsaCheck.error}`);
