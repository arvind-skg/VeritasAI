...
export async function withAttestation(agentId, eventType, fn, toMetadata = () => ({})) {
    // The agent's actual decision — ALWAYS returned, no matter what
    const result = await fn();
    try {
        const { evidence } = await recordEvent(eventType, toMetadata(result), { output: result });
        ...
        await saveReceipt(agentId, eventType, null, "recording_failed", String(err));
        console.error(`[VeritasAI] evidence recording failed for ${eventType}:`, err);
    }
    return result;
}
