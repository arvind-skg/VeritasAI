...
router.post("/", agentAuth, async (req, res) => {
    ...
    try {
        const { decision, event_type, eventType, input_data, inputData, output, metadata, risk_level, riskLevel, model_info, modelInfo, } = req.body;
        if (!decision) {
            res.status(400).json({
        ...
        try {
            const recordResult = await defaultProvider.record(resolvedEventType, resolvedMetadata, payloads);
