...
import { createOrganization, createUser, getUserByEmail, getUserById, getOrganizationStats, createAgent, createAuditLog, } from "../services/db.js";
import { hashPassword, verifyPassword, generateToken, } from "../services/security.js";
...
router.post("/register", async (req, res) => {
    try {
        ...
        const user = await createUser(organization.id, email, passwordHash, name?.trim() || "Administrator", "ADMIN");
        ...
        // 4. Audit Log
        await createAuditLog(organization.id, "ORGANIZATION_REGISTERED", "ORGANIZATION", organization.id, user.id, { orgName, email });
        // 5. Issue JWT
