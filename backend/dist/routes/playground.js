...
import { verifyActionChain, verifyConsistency, verifyHybridSignature, generateHybridSignature, verifyExternalTimestamp, } from "../services/cryptography/index.js";
const router = Router();
const SCENARIOS = [
    {
        id: "MODIFY_DECISION",
...
router.post("/attack", async (req, res) => {
    try {
        const { attackType, eventId } = req.body;
        ...
        const baselineChain = (targetEvent.actionChain || []);
        let tamperedState = JSON.parse(JSON.stringify(targetEvent));
        let detected = false;
        ...
        switch (attackType) {
            case "MODIFY_DECISION": {
                ...
                const genuineSig = generateHybridSignature({ id: targetEvent.id, decision: originalDecision }, dummySeed);
                const sigVerification = verifyHybridSignature({ id: targetEvent.id, decision: forgedDecision }, genuineSig, dummySeed);
            ...
            case "SWAP_MODEL": {
                const originalModel = targetEvent.modelProvenance?.model || "CreditGPT-Underwriter v2.1";
            ...
            case "MODIFY_POLICY": {
                const originalPolicyHash = targetEvent.policyProof?.policyHash || "8a91b2c4e5f60718293a4b5c6d7e8f90";
            ...
            case "SWAP_HUMAN_APPROVAL": {
                const originalReviewer = targetEvent.humanApproval?.reviewer || "Senior Risk Officer #4481";
