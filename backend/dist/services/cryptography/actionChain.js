...
export function computeStageHash(prevHash, stage, payload, timestamp) {
...
export function buildActionChain(stages) {
    let prevHash = null;
    const result = [];
...
export function verifyActionChain(chain) {
    ...
    for (let i = 0; i < chain.length; i++) {
        ...
        const calculatedHash = computeStageHash(expectedPrevHash, stage.stage, stage.payload, stage.timestamp);
...
export function synthesizeDefaultActionChain(params) {
    ...
    const input = (params.inputData && typeof params.inputData === "object" ? params.inputData : {});
    const output = (params.outputData && typeof params.outputData === "object" ? params.outputData : {});
