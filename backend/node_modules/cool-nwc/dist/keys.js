...
import { concatBytes, toBase64Field, utf8 } from "./codec.js";
const ML_DSA_SEED_BYTES = 32;
function deriveEd25519Secret(seed) {
 ...
 */
export function generateKeypair(keyId, options = {}) {
...
export function directoryFromKeypair(key) {
    return { [key.keyId]: key.directoryEntry };
}
/** Merge several keypairs' directory entries into one key directory. */
export function mergeDirectories(...keys) {
    const directory = {};
    for (const key of keys)
        directory[key.keyId] = key.directoryEntry;
    return directory;
}
