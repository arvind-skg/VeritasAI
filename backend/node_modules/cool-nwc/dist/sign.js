...
import { fromBase64Field, toBase64Field } from "./codec.js";
export const SIGNATURE_ALG = "ml-dsa-65+ed25519";
/** Produce a hybrid signature block over `message` using both secret keys. */
export function hybridSign(message, key) {
 ...
 */
export function hybridVerify(message, signature, entry) {
    let mlDsaOk = false;
    let ed25519Ok = false;
    try {
        mlDsaOk = ml_dsa65.verify(fromBase64Field(entry.ml_dsa_pub), message, fromBase64Field(signature.ml_dsa));
    }
    catch {
        mlDsaOk = false;
    }
    try {
        ed25519Ok = ed25519.verify(fromBase64Field(signature.ed25519), message, fromBase64Field(entry.ed25519_pub));
    }
    catch {
        ed25519Ok = false;
    }
