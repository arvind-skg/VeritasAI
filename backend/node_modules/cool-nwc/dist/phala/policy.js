...
function matches(rule, input) {
    const m = rule.when;
    if (m.kinds && !m.kinds.includes(input.kind))
        return false;
    if (m.environments && !m.environments.includes(input.environment))
        return false;
    if (m.actorMethods && !m.actorMethods.includes(input.actor.method))
        return false;
    if (m.ref) {
        if (m.ref.startsWith("re:")) {
            if (!new RegExp(m.ref.slice(3)).test(input.ref))
                return false;
        }
        else if (!input.ref.includes(m.ref))
            return false;
    }
    if (m.minApprovers !== undefined && input.approvers.length < m.minApprovers)
        return false;
    if (m.maxApprovers !== undefined && input.approvers.length > m.maxApprovers)
        return false;
    if (m.minRisk !== undefined && (input.risk ?? 0) < m.minRisk)
        return false;
    if (m.labels && !m.labels.every((label) => (input.labels ?? []).includes(label)))
        return false;
    return true;
}
/** Deterministic commitment to a rule set, so a record names the rules it met. */
export function policyHash(policy) {
    return mhSha256(canonicalCbor(policy));
}
/**
...
export function evaluate(policy, input) {
    ...
    const winner = matched.reduce((worst, rule) => SEVERITY[rule.decision] > SEVERITY[worst.decision] ? rule : worst);
    return {
 ...
 */
export function approvalFrom(outcome, approvers) {
    return {
        policy_id: outcome.rule ?? "no-matching-rule",
        decision: outcome.decision === "escalate"
            ? "rejected"
            : outcome.decision,
        approvers: [...approvers],
    };
