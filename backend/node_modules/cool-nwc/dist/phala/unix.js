...
export function isSocketPath(endpoint) {
 ...
 */
export function unixFetch(socketPath) {
    return (input, init = {}) => new Promise((resolve, reject) => {
        ...
        req.on("error", (error) => {
            ...
            if (error.code === "ENOENT") {
                reject(new Error(`no dstack guest agent at ${socketPath} — are you inside a CVM? ` +
                    "Outside one, unset DSTACK_ENDPOINT and CooL runs its labelled simulator."));
                return;
            }
            if (error.code === "EACCES") {
                reject(new Error(`permission denied on ${socketPath} — the container needs the socket mounted ` +
 ...
 */
export function transportFor(endpoint) {
