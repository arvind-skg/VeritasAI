 ...
 * the SDK.
 */
import { domainOrder, verifyReceiptV2, withTrustedKeys, } from "./phala/verify.js";
 ...
 */
export function verifyEvidence(evidence, options = {}) {
...
export function formatVerdict(verdict) {
    ...
    const rows = [rule, line("COOL VERIFIER"), rule];
    if (verdict.subject) {
        rows.push(line(`subject     ${verdict.subject.subject}`), line(`record id   ${verdict.subject.record_id}`), line(`signer      ${verdict.subject.key_id}`), line(`runtime     ${verdict.subject.tee}`), line());
    }
