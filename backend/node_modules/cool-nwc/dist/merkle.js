...
import { bytesEqual, concatBytes } from "./codec.js";
...
export function inclusionProof(leafHashes, m) {
    const n = leafHashes.length;
    if (m < 0 || m >= n)
        throw new RangeError(`leaf index ${m} out of range for size ${n}`);
    if (n === 1)
        return [];
    const k = largestPowerOfTwoBelow(n);
    if (m < k) {
 ...
 */
export function rootFromInclusionProof(leafHashValue, leafIndex, treeSize, auditPath) {
    ...
    for (const sibling of auditPath) {
        if (sn === 0)
            throw new RangeError("audit path too long for tree size");
        if (fn % 2 === 1 || fn === sn) {
    ...
    if (sn !== 0)
        throw new RangeError("audit path too short for tree size");
    return hash;
}
/**
 ...
 */
export function verifyInclusion(leafHashValue, leafIndex, treeSize, auditPath, expectedRoot) {
    let computed;
    try {
...
export function consistencyProof(leafHashes, m) {
    const n = leafHashes.length;
    if (m < 0 || m > n)
        throw new RangeError(`invalid first size ${m} for second size ${n}`);
    if (m === 0 || m === n)
        return [];
    return subproof(m, leafHashes, true);
}
function subproof(m, hashes, b) {
 ...
 */
export function verifyConsistency(m, n, firstRoot, secondRoot, proof) {
    if (m === 0)
        return true;
    if (m === n)
