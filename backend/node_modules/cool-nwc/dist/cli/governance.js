...
import { DEFAULT_POLICY, buildAuditPack, coverage, evaluate, verifyAuditPack, } from "../phala/index.js";
import { loadReceipts } from "./workspace.js";
import { Progress, bar, c, columns, fields, g, out, panel, pad } from "./tty.js";
import { paragraph } from "./help.js";
const flag = (args, name) => {
    const index = args.indexOf(`--${name}`);
    return index >= 0 ? args[index + 1] : undefined;
};
/** The rule set in force. One place, so the CLI cannot print a different one. */
export const ACTIVE_POLICY = DEFAULT_POLICY;
/* ── policy ───────────────────────────────────────────────────────────── */
export function policyCommand(args) {
    if (args[0] === "test") {
        const kind = (args[1] ?? "prompt");
        ...
        const tone = outcome.decision === "rejected" || outcome.decision === "escalate" ? c.yellow : c.green;
        panel("policy test", [
        ...
        out();
        out(`  ${c.faint("The strictest matching rule wins, so a permissive rule added later cannot override a stricter one.")}`);
        out();
        return 0;
    }
    out();
    ...
    for (const rule of ACTIVE_POLICY.rules) {
        const tone = rule.decision === "rejected" || rule.decision === "escalate" ? c.yellow : c.green;
        out(`  ${c.bold(c.brand(rule.id))}  ${rule.title}`);
        out(`    ${c.grey("decision")}  ${tone(rule.decision)}`);
        const when = [];
        if (rule.when.environments)
            when.push(`env ∈ [${rule.when.environments.join(", ")}]`);
        if (rule.when.kinds)
            when.push(`kind ∈ [${rule.when.kinds.join(", ")}]`);
        if (rule.when.minApprovers !== undefined)
            when.push(`approvers ≥ ${rule.when.minApprovers}`);
        if (rule.when.minRisk !== undefined)
            when.push(`risk ≥ ${rule.when.minRisk}`);
        if (rule.when.ref)
            when.push(`ref ~ ${rule.when.ref}`);
        out(`    ${c.grey("when")}      ${when.join("  ·  ") || c.faint("always")}`);
        if (rule.obligations?.length) {
            out(`    ${c.grey("covers")}    ${rule.obligations.join(", ")}`);
        }
        if (rule.because)
            out(`    ${c.faint(rule.because)}`);
        out();
    }
    out(`  ${c.grey("fallback")}  ${c.yellow(ACTIVE_POLICY.fallback ?? "escalate")} ${c.faint("— anything unmatched needs a human")}`);
    out();
...
/* ── compliance ───────────────────────────────────────────────────────── */
export function complianceCommand(workspace, args) {
    ...
    for (const row of shown) {
        ...
        out(`    ${bar(row.records, receipts.length, width)} ${pad(String(row.records), 4)}` +
            `${c.faint(`records · ${row.obligation.satisfiedBy}`)}`);
        out();
    }
    const covered = rows.filter((row) => row.covered).length;
    out(`  ${covered} of ${rows.length} obligations have evidence behind them` +
        c.faint("  — counted from receipts, never asserted"));
    out();
    return 0;
}
/* ── pack ─────────────────────────────────────────────────────────────── */
export async function packCommand(workspace, args) {
    ...
    if (sub === "verify") {
        ...
        if (verdict.ok)
            progress.succeed(`every record in ${path} verifies`);
        else
            progress.fail(`${verdict.failed} of ${verdict.total} records FAILED`);
        fields([
        ...
        out();
        out(`  ${c.faint("The summary above was recomputed from the receipts, not read from the pack.")}`);
        out();
    ...
    out();
    paragraph(c.faint("Self-contained: every receipt, the keys to check them, the enclave measurement and " +
