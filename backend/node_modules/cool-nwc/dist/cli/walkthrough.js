...
import { DEFAULT_POLICY, buildAuditPack, disclose, evaluate, verifyAuditPack, verifyDisclosure, } from "../phala/index.js";
import { saveReceipt, verify } from "./workspace.js";
import { Progress, c, columns, g, out, panel, rule } from "./tty.js";
import { printVerdict } from "./commands.js";
...
function pacer() {
    ...
    return {
        next: (label) => new Promise((resolve) => {
...
export async function walkthrough(workspace) {
    const step = pacer();
    ...
    say("CooL records two things: what a model did, and what you changed about it. This is " +
    ...
    say(`Signed with ${c.bold(receipt.record.signature.key_id)} — a key derived inside the ` +
    ...
    say("This is the function an auditor runs. Offline, no account, no network: recompute " +
    ...
    say(c.faint("Two domains say `simulated` rather than `pass`: this machine has no confidential " +
    ...
    say("Evidence is only worth what its failures are worth. One character of one hash is " +
        "about to change — the smallest edit anyone would attempt.");
    out();
    const forged = JSON.parse(JSON.stringify(receipt));
    const change = forged.record.change;
    ...
    say("Binding fails because the contents no longer match the commitment, and the " +
    ...
    say("An approval that can be edited afterwards is not an approval. So policy is " +
    ...
    out(`  ${c.green(g.pass)} prompt change, two approvers    → ${c.bold(approved.decision)} ${c.faint(`(${approved.rule})`)}`);
    out(`  ${c.yellow(g.warn)} widening an agent's permissions → ${c.bold(refused.decision)} ${c.faint(`(${refused.rule})`)}`);
    out();
    say(c.faint(`Why: ${DEFAULT_POLICY.rules.find((r) => r.id === refused.rule)?.because ?? ""}`));
    say("The strictest matching rule wins, so a permissive rule added later cannot quietly " +
    ...
    say("The receipt contains no plaintext, which is what lets you publish it. So when a " +
    ...
    out();
    const { loadReceipts } = await import("./workspace.js");
    ...
    out(`  ${c.grey("verified")}     ${packVerdict.failed === 0
        ? c.green(String(packVerdict.verified))
        : `${c.green(String(packVerdict.verified))} ${c.red(`· ${packVerdict.failed} failed`)}`}`);
    out(`  ${c.grey("obligations")}  ${packVerdict.obligationsCovered} of ${packVerdict.obligationsTotal} covered by evidence`);
    out();
    say(c.faint("Build one any time with `cool pack build --out audit.json`."));
    out();
    rule("done");
    say(`${c.bold("That is the whole model.")} Records that fail loudly when they are wrong, ` +
