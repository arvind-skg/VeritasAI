...
import { attachWitness, cosign, countWitnesses, disclosableFields, disclose, environmentOf, query, subjectOf, verifyDisclosure, } from "../phala/index.js";
import { generateKeypair } from "../keys.js";
import { loadReceipts, saveReceipt } from "./workspace.js";
import { Progress, c, fields, g, out, panel, pad } from "./tty.js";
/* ── records ──────────────────────────────────────────────────────────── */
...
export function records(workspace, args) {
    ...
    out();
    out(`  ${c.grey(pad("WHEN", 20))}${c.grey(pad("KIND", 18))}${c.grey(pad("SUBJECT", 34))}${c.grey("ENV")}`);
    for (const receipt of found) {
        ...
        const mark = decision === "rejected" ? c.red(g.warn) : decision ? c.green(g.pass) : c.faint(g.dot);
        out(`  ${mark} ${pad(receipt.record.time.issued_at.slice(0, 19).replace("T", " "), 20)}` +
 ...
 */
export function discloseCommand(workspace, args) {
    const stored = loadReceipts(workspace?.root);
    const checkPath = flag(args, "check");
    if (checkPath) {
        const disclosure = JSON.parse(readFileSync(checkPath, "utf8"));
    ...
    const entry = !target || target === "last"
        ? stored.at(-1)
    ...
    if (!field) {
        ...
        for (const f of disclosableFields(entry.receipt))
            out(`    ${c.brand(f)}`);
        out();
        return 1;
    }
    try {
        const disclosure = disclose(entry.receipt, field, value);
        ...
        out();
        out(`  ${c.faint("Anyone can check this with")} ${c.brand("cool disclose --check <file>")}${c.faint(" — it needs no enclave.")}`);
        out();
        return 0;
    }
    catch (error) {
 ...
 */
export function witnessCommand(workspace, args) {
    const stored = loadReceipts(workspace?.root);
    ...
    if (sub === "list") {
        ...
        out();
        if (external === 0) {
            out(`  ${c.faint("Nobody independent has co-signed yet. Try")} ${c.brand("cool witness cosign --key auditor")}`);
            out();
        }
        return 0;
    }
    ...
    const key = generateKeypair(`witness-${name}`, {
        seed: new Uint8Array(Array.from({ length: 32 }, (_, i) => (name.charCodeAt(i % name.length) + i * 7) & 0xff)),
    });
    ...
    progress.succeed(`co-signed ${heads.size} tree head${heads.size === 1 ? "" : "s"} as ${c.bold(key.keyId)}`);
    fields([
...
export async function logCommand(workspace, args) {
    ...
    if (workspace.log) {
        ...
        if (previous && previous.tree_size !== sth.tree_size) {
            out(`  ${c.grey("last head")}   size ${previous.tree_size} at ${previous.timestamp.slice(0, 19)}`);
        }
        if (consistencyFrom) {
            const from = Number(consistencyFrom);
            const proof = workspace.log.consistency(from);
            out();
            out(`  ${c.bold(`consistency ${from} → ${sth.tree_size}`)}  ${c.faint(`${proof.length} hash${proof.length === 1 ? "" : "es"}`)}`);
            for (const node of proof)
                out(`    ${c.faint(node)}`);
            out();
            out(`  ${c.faint("Hand this and an older tree head to anyone who kept one: it proves the log only grew.")}`);
        }
    }
    else {
...
export function receiptsOf(workspace) {
