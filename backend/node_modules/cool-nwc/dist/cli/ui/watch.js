...
export function shouldWatch(relPath) {
...
export function kindFor(relPath) {
    ...
    if (/(^|\/)(policies|policy|governance)\//.test(p) || name.endsWith(".rego"))
        return "policy";
    if (/(^|\/)(datasets?|data|fixtures)\//.test(p))
        return "dataset";
    if (/(tool|function)s?\.(json|ya?ml|ts|js)$/.test(name))
        return "tool";
    if (/(permission|scope|role|acl)s?\./.test(name))
        return "agent-permission";
    if (/(^|\/)(models?)\//.test(p) || /model\.(json|ya?ml)$/.test(name))
        return "model";
    if (/\.(json|ya?ml|toml|ini|env)$/.test(name))
        return "params";
    return "prompt";
}
/**
 ...
 */
export function refFor(relPath) {
...
export function watchProject(options) {
    ...
    const watchDir = (dir, depth) => {
        if (watchers.has(dir) || depth > 12)
            return;
        let handle;
        try {
            ...
            // covered.
            options.onError?.(`cannot watch ${relative(root, dir) || "."}: ${error.message}`);
            return;
        }
        handle.on("error", () => {
            watchers.delete(dir);
    ...
    try {
        ...
        options.onError?.(`live watching unavailable on this filesystem (${error.message})`);
    }
    if (watchers.size === 0) {
