...
import { c, g, out } from "../tty.js";
import { committedContents, gitContext, trackedFiles } from "./git.js";
import { shouldWatch, watchProject } from "./watch.js";
import { Console } from "./server.js";
...
export async function ui(workspace, argv) {
    const parsed = parse(argv);
    if (!existsSync(parsed.root)) {
        out(`  ${c.red(g.fail)} no such folder: ${c.bold(parsed.root)}`);
        return 1;
    }
    const args = { ...parsed, root: canonical(parsed.root) };
    ...
    const active = workspace && workspace.root === args.root ? workspace : await openWorkspace(args.root);
    ...
    try {
        ...
        out(`  ${c.red(g.fail)} could not listen on ${args.host} — ${error.message}`);
        return 1;
    }
    ...
    if (git.root) {
        out(`  ${c.faint("git")}       ${git.branch ?? "detached"}${git.commit ? ` · ${git.commit}` : ""}${git.author ? ` · ${git.author}` : ""}`);
    }
    else {
    ...
    out(`  ${c.faint("evidence")}  ${existing} existing · log ${active.cool.plane.logSize} · ${c.dim(join(args.root, ".cool"))}`);
    out(`  ${c.faint("watching")}  ${seeded} files`);
    out();
    if (hardware && verified) {
        out(`  ${c.green(g.pass)} ${c.bold("hardware attestation active")} — receipts can reach ${c.bold("pass")}`);
    }
    else if (hardware) {
        out(`  ${c.yellow(g.warn)} guest agent answering, but no ${c.bold("QUOTE_VERIFIER_URL")} — quotes are reported, not verified`);
    }
    else {
        out(`  ${c.yellow(g.warn)} ${c.bold("simulated")} — no confidential VM. Every receipt says so in its own body.`);
        out(`    ${c.faint("to reach a real quote:")} ${c.dim("cool help deploy")}`);
    }
    if (movedFrom !== null) {
        out(`  ${c.faint(`port ${movedFrom} was busy — using ${url.split(":").pop()} instead`)}`);
    }
    out();
