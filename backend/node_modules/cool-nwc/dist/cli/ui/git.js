 ...
 */
export function committedContents(cwd, relativePath) {
 ...
 */
export function actorFrom(context) {
    if (context.authorEmail)
        return { id: `user:${context.authorEmail}`, method: "session" };
    if (context.author)
        return { id: `user:${context.author}`, method: "session" };
    return { id: "user:local", method: "session" };
}
/** Labels the policy engine can match on, and an auditor can read. */
export function labelsFrom(context, repoName) {
    const labels = [`repo:${repoName}`];
    if (context.branch)
        labels.push(`branch:${context.branch}`);
    if (context.commit)
        labels.push(`commit:${context.commit}`);
    return labels;
}
