 ...
 */
import { verifyReceiptV2 } from "../phala/index.js";
import { isSocketPath } from "../phala/unix.js";
import { verify } from "./workspace.js";
import { c, g, out, panel, rule } from "./tty.js";
import { paragraph } from "./help.js";
const mark = (ok) => (ok ? c.green(g.pass) : c.yellow(g.warn));
export async function wire(workspace) {
    const steps = [];
    ...
    steps.push({
        ...
        detail: endpoint
            ? `${endpoint}${isSocketPath(endpoint) ? " (unix socket)" : ""} · report_data ${boundToKey ? "commits to " + workspace.cool.plane.keys.record.keyId : "DOES NOT match the signing key"}`
    ...
    steps.push({
        ...
        ok: quoteInRecord,
        detail: quote
            ? `format ${quote.format} · root ${quote.root}${quote.root === "cool-sim-root" ? c.yellow("  ← CooL's simulator, not a vendor") : ""}`
            : "no quote at all",
        fix: [
    ...
    if (!blocking) {
        ...
        out(`    ${c.brand("witnesses")} ${c.faint("— an independent party co-signs the head:")} ${c.dim("cool witness cosign --key auditor")}`);
        out(`    ${c.brand("anchor")}    ${c.faint("— commit the head to Bitcoin:")} ${c.dim("cool anchor submit")} ${c.faint("then")} ${c.dim("cool anchor upgrade")} ${c.faint("an hour later")}`);
        out();
        return 0;
    }
    ...
    paragraph(`${c.bold("Measurement pin")} — if the deployed image's MRTD/RTMR differs from the value you ` +
        "pinned, the enclave domain fails. That is correct: it means the running code is not the " +
        "code you approved.");
    out();
    paragraph(`${c.bold("TCB status")} — if Intel reports the platform's firmware or microcode as out of ` +
        "date, the quote verifies but the platform is stale, and the verifier says so rather than " +
        "passing. Patch the host.");
    out();
    paragraph(`${c.bold("report_data binding")} — a quote that does not commit to the signing key proves ` +
