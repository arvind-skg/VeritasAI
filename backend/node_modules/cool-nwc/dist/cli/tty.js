...
export const useColor = isTTY && !process.env["NO_COLOR"] && process.env["TERM"] !== "dumb" && !process.env["CI"];
/**
 ...
 */
export const useUnicode = process.platform !== "win32" ||
    Boolean(process.env["WT_SESSION"] ?? process.env["TERM_PROGRAM"] ?? process.env["ConEmuANSI"]);
const wrap = (open, text) => useColor ? `${ESC}[${open}m${text}${ESC}[0m` : text;
const fg = (code) => (text) => wrap(`38;5;${code}`, text);
export const c = {
    bold: (t) => wrap("1", t),
    dim: (t) => wrap("2", t),
    inverse: (t) => wrap("7", t),
...
export function padStart(text, to) {
    const gap = to - width(text);
    return gap > 0 ? " ".repeat(gap) + text : text;
}
export function truncate(text, to) {
    if (width(text) <= to)
        return text;
...
export function panel(title, lines, tone = c.brand) {
    const w = columns();
    const inner = w - 4;
    const heading = ` ${title} `;
    out(tone(g.tl + g.h) +
...
export function bar(value, max, cells = 24) {
    if (max <= 0)
        return c.faint(g.light.repeat(cells));
...
export function sparkline(values) {
    if (values.length === 0)
        return c.faint("no data");
    const max = Math.max(...values, 1);
    return c.blue(values
        .map((value) => g.spark[Math.min(g.spark.length - 1, Math.round((value / max) * (g.spark.length - 1)))] ?? " ")
        .join(""));
}
