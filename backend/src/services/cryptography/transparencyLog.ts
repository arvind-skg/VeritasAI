...
export interface InclusionProof {
  ...
  treeSize: number;
  leafHash: string;
  auditPath: Array<{ direction: "left" | "right"; hash: string }>;
}
...
export function generateInclusionProof(
  ...
  const auditPath: Array<{ direction: "left" | "right"; hash: string }> = [];
  let idx = leafIndex;
 ...
 */
export function generateSignedTreeHead(
  leafHashes: string[],
  signingKeyHex: string = "simulated_ed25519_key"
): SignedTreeHead {
...
export function verifyConsistency(
  firstTreeLeaves: string[],
  secondTreeLeaves: string[]
): {
