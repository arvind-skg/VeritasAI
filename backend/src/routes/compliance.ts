...
const FRAMEWORKS: Record<RegulatoryFramework, FrameworkMeta> = {
  EU_AI_ACT: {
...
router.post("/generate", combinedAuth, async (req: Request, res: Response) => {
  try {
    ...
    const frameworkKey = (req.body.framework || "EU_AI_ACT") as RegulatoryFramework;
...
${events
  ...
  .map(
    (e) =>
      `| \`${e.id.slice(0, 12)}...\` | ${e.eventType} | **${e.decision || "PROCESSED"}** | ${e.riskLevel} | ${(e.modelProvenance as any)?.version || "v2.1"} | ${(e.humanApproval as any)?.reviewer || "Senior Auditor"} |`
  )
  .join("\n")}
