...
router.post("/", combinedAuth, async (req: Request, res: Response) => {
  try {
    ...
    await createAuditLog(
      ...
      agent.id,
      req.user?.userId,
      { name: agent.name, environment: agent.environment }
    );
 ...
 */
router.patch("/:id", orgAuth, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get("/:id/credentials", orgAuth, async (req: Request, res: Response) => {
  try {
    const agentId = String(req.params.id);
 ...
 */
router.post("/:id/credentials", orgAuth, async (req: Request, res: Response) => {
  try {
    const agentId = String(req.params.id);
    ...
    await createAuditLog(
      ...
      credential.id,
      req.user?.userId,
      { agentId, keyPrefix: credential.keyPrefix }
    );
 ...
 */
router.post("/:id/credentials/:credId/rotate", orgAuth, async (req: Request, res: Response) => {
  try {
    const agentId = String(req.params.id);
 ...
 * Revoke an API key.
 */
router.delete("/:id/credentials/:credId", orgAuth, async (req: Request, res: Response) => {
  try {
    const agentId = String(req.params.id);
    const credId = String(req.params.credId);
